...
- Assessment A and B MUST run as two isolated sub-agents whenever `spawn_agent` is exposed and current instructions permit bounded delegation. Running them inline in this context is degraded. Inline is allowed when sub-agents are unavailable or prohibited by current instructions.
...
Codex sub-agent gate (overrides the default above):
- If `spawn_agent` is exposed and current instructions permit bounded delegation, spawn A and B immediately without a routine permission checkpoint.
- If current instructions prohibit delegation, run sequentially and lead with `⚠️ DEGRADED: single-context (delegation prohibited by current instructions)`.
- If `spawn_agent` is not exposed, run sequentially and lead with `⚠️ DEGRADED: single-context (spawn_agent unavailable in this session)`.
- If spawning fails, run sequentially and lead with `⚠️ DEGRADED: single-context (sub-agent spawn failed: <exact error>)`.
